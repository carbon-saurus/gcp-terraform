...
data "google_dns_managed_zone" "prod_carbontrack_app_zone" {
  project = var.project_id
...
resource "google_dns_record_set" "root_domain" {
  project      = var.project_id
  managed_zone = data.google_dns_managed_zone.prod_carbontrack_app_zone.name
...
# 메인 도메인 A 레코드 생성 (www.carbonsaurus.net)
resource "google_dns_record_set" "prod_carbontrack_app_a_record" {
  project      = var.project_id
  managed_zone = data.google_dns_managed_zone.prod_carbontrack_app_zone.name
  name         = "www.${var.dns_domain_name}"  # www.carbonsaurus.net
  type         = "A"
  ttl          = 300
  rrdatas      = [google_compute_global_address.ingress_static_ip.address]
...
# api 서브도메인 A 레코드 생성
resource "google_dns_record_set" "prod_api" {
...
resource "google_dns_record_set" "prod_admin" {
